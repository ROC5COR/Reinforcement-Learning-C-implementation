...
void matrixDotProduct(MAT *mat1, MAT *mat2, MAT *matOut){
    if(mat1->matN != mat2->matM || 
       mat2->matN != matOut->matN ||
         mat1->matM != matOut->matM){
        printf("Error(Product) : Size won't works(%d,%d) and (%d,%d) into (%d,%d)\n",mat1->matM,mat1->matN,mat2->matM,mat2->matN, matOut->matM, matOut->matN);
        matOut = NULL;
        return;
    }
    ...
    for(i = 0; i < mat1->matM; i++){
        for(j = 0; j < mat2->matN; j++){
            ((VAR**)matOut->mat)[i][j] = 0;
            for(k = 0; k < mat2->matM; k++){
                ((VAR**)matOut->mat)[i][j] += ((VAR**)mat1->mat)[i][k] * ((VAR**)mat2->mat)[k][j];
            }
        }
    }
}
...
MAT* createMatrix_float(uint16_t m, uint16_t n){
    //printf("creating matrix with size (%d,%d)\n", m, n);
    MAT *out = (MAT*)malloc(sizeof(MAT));
    VAR **mat = NULL;
    mat = (VAR**)malloc(sizeof(VAR*)*m);
    
    if(out == NULL){
        printf("Error while creating matrix (1)\n");
        return NULL;
    }
    
    if(mat == NULL){
        printf("Error while creating matrix (2)\n");
        return NULL;
    }
    ...
    for(i = 0; i < m; i++){
        ...
        if(mat[i] == NULL){
            printf("Error while creating matrix (3)\n");
            return NULL;
        }
...
void matrixMultiply(MAT *mat1, MAT *mat2, MAT *matOut){
    if(mat1->matM != mat2->matM || mat2->matM != matOut->matM ||
        mat1->matN != mat2->matN || mat2->matN != matOut->matN)
    {
      printf("Error in matrixMultiply : bad dimension (%d,%d) (%d,%d) (%d,%d)\n", mat1->matM, mat1->matN, mat2->matM, mat2->matN, matOut->matM, matOut->matN);
      return;
    }
